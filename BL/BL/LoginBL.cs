...
namespace BL.BL
{
    public class LoginBL
    {
        ...
        public TAResponse LoginShop(string email, string pass)
        {
            string sqlgetUser = Properties.Resources.login.Replace("{{email}}", $"'{email}'").Replace("{{password}}", $"'{pass}'");
            string sqlgetUserAndShop = Properties.Resources.loginShop.Replace("{{email}}", $"'{email}'").Replace("{{password}}", $"'{pass}'");
            ...
            {
                var userShop = dl.getUser(sqlgetUserAndShop);
                if(userShop == null)
                {
                    return new TAResponse()
                    {
                        data = Properties.Resources.errNotExitShop,
                        is_success = false,
                        status = stat.ClientError,
                    };
                }
